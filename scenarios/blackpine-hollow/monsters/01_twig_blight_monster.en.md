# Twig Blight

---

### Encounter Reference

**Default quantity:** 4-6 (see scaling)
**Wave:** Moonlit creek (Scene 5), combat begins in Scene 6
**Challenge Rating CR:** —
**Stat source:** Homebrew

### Quick Tactics

**Behavior:** Ambush from sticks; half defeated -> flee or collapse.
**Weakness:** Fire (optional double damage).

---

### Stat Block

**Type:** Small plant, unaligned
**Armor Class AC:** 13
**Hit Points HP:** 4
**Speed:** 20 ft.

**Senses:** passive Perception 9
**Languages:** —
**Challenge Rating CR:** —

### Main Stats

| Stat | Score | Modifier |
|---|--:|--:|
| Strength | 6 | −2 |
| Dexterity | 13 | +1 |
| Constitution | 12 | +1 |
| Intelligence | 1 | −5 |
| Wisdom | 3 | −4 |
| Charisma | 1 | −5 |

### Traits

- **Plant creature.** Does not think like an animal or person.
- **Dry wood body.** Fire is especially dangerous; optional rule: fire damage is doubled.
- **Ambusher.** Looks like dead sticks until it moves.

### Actions

| Action | Hit | Damage | Notes |
|---|---|---|---|
| Claws | +3 | 1d4+1 piercing | Reach 5 ft. |

### DM Notes

- Scene anchor: `05_moonlit_creek` leads into `06_small_combat`.
- **Scaling:** 3 players -> 4 twig blights; 4 players -> 5 twig blights; 5 players -> 6 twig blights.
- When half the blights are defeated, the rest can flee or collapse into ordinary sticks if pacing feels slow.
- Use fire weakness only if you want a dramatic beginner-friendly moment.

---
